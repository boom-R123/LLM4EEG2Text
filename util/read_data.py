...
for mat_file in tqdm(mat_files):
    ...
    for sent in matdata:
        ...
        # print(len(sent.wordbounds)) #每个word的上下左右坐标
        # print(sent.allFixations.duration) # 眼睛注视的坐标，以及持续时间，可以通过坐标判断在注视哪一个单词，或者是在注视超出单词外的部分，用于与EEG信号做对齐
        print(np.array(sent.rawData).shape)
        word_data = sent.word
        if not isinstance(word_data, float):
            ...
            continue
        print(len(sent_obj['word_tokens_all']))
